...
const key_generator_1 = require("./key_generator");
describe('Mnemonic validation', () => {
    it('should generate 24 word mnemonic', () => {
        const mnemonic = key_generator_1.ReactNativeBip39MnemonicGenerator.generateMnemonic();
    ...
    it('should generate an expected private key for a mnemonic', () => {
        ...
        for (let i = 0; i < mnemonics.length; i++) {
            expect(bip39_1.validateMnemonic(mnemonics[i])).toBeTruthy();
            const actualPrivateKey = key_generator_1.ReactNativeBip39MnemonicGenerator.mnemonicToSeedHex(mnemonics[i]);
            expect(actualPrivateKey).toEqual(expectedPrivateKeys[i]);
        }
    });
});
//# sourceMappingURL=key_generator.test.js.map