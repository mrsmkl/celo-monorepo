...
export function addLocalAccount(web3: Web3, privateKey: string): Web3 {
  ...
  if (existingProvider instanceof Web3ProviderEngine) {
    debug('Existing provider is already a Web3ProviderEngine')
...
function isAccountAlreadyAdded(
  ...
  for (const localAccountAddress of localAccountAddresses) {
    if (alreadyAddedAddresses.indexOf(localAccountAddress) < 0) {
      debug(
        'Account %o is not already added, ' + 'existing accounts are %o',
        localAccountAddress,
  ...
  debug('Account %o is already added', localAccountAddresses)
  return true
}
